...
namespace TimerBot.Bot.Handlers
{
    public class ClientLogHandler
    {
        ...
        private async Task ShardDisconnectedAsync(Exception exception, DiscordSocketClient shard)
        {
            try
            {
                var channel = _client.GetGuild(Convert.ToUInt64(ConfigData.Data.BaseServer))
                    .GetTextChannel(Convert.ToUInt64(ConfigData.Data.ExceptionChannel));
                await channel.SendMessageAsync(
                    $"<:RedStatus:519932993343586350> Shard: `{shard.ShardId}` Disconnected with the reason {exception.Message}");
            }
            catch (Exception)
            {
        ...
        private async Task ShardConnectedAsync(DiscordSocketClient shard)
        {
            try
            {
                ...
                var channel = _client.GetGuild(Convert.ToUInt64(ConfigData.Data.BaseServer))
                    .GetTextChannel(Convert.ToUInt64(ConfigData.Data.ExceptionChannel));
                await channel.SendMessageAsync(
                    $"<:GreenStatus:519932750296514605> Shard: `{shard.ShardId}` Connected with {shard.Latency}ms");
            }
            catch (Exception)
            {
                // Logs the message to a txt file if it was unable to send the message.
        ...
        private async Task ShardLatencyUpdatedAsync(int oldPing, int updatePing, DiscordSocketClient shard)
        {
            ...
            try
            {
                var channel = _client.GetGuild(Convert.ToUInt64(ConfigData.Data.BaseServer))
                    .GetTextChannel(Convert.ToUInt64(ConfigData.Data.ExceptionChannel));
                await channel.SendMessageAsync(
                    $"Shard: `{shard.ShardId}` Latency update from **{oldPing}** ms to **{updatePing}** ms");
            }
            catch (Exception e)
            {
                // Logs the message to a txt file if it was unable to send the message.
        ...
        private async Task ClientJoinedGuild(SocketGuild guild)
        {
            try
            {
                var channel = _client.GetGuild(Convert.ToUInt64(ConfigData.Data.BaseServer))
                    .GetTextChannel(Convert.ToUInt64(ConfigData.Data.ExceptionChannel));
                await channel.SendMessageAsync(
                    $"Joined server: {guild.Name}, Id: {guild.Id}, MemberCount: {guild.MemberCount}.");
            }
            catch (Exception e)
            {
                // Logs the message to a txt file if it was unable to send the message.
                _logger.LogException($"Joined server: {guild.Name}, Id: {guild.Id}, MemberCount: {guild.MemberCount}.",
                    e);
            }
        }
